...
__global__ void fastpls_candidate_knn_scores_prefix_kernel(const double* Ttest,
                                                           const double* Ttrain,
  ...
  const int out_col = cand_slot + slice * top_m;
  const int cls = candidates[row + out_col * ntest];
  if (cls < 1 || cls > n_classes) {
    out_scores[idx] = -INFINITY;
    return;
  }
  ...
  if (kk < 1) kk = 1;
  if (kk > kdim) kk = kdim;
  const double test_n2 = test_norm2[row + (kk - 1) * ntest];
  ...
  for (int pos = start; pos < end; ++pos) {
    const int tr = class_indices[pos];
    if (tr < 0 || tr >= ntrain) continue;
    const double train_n2 = train_norm2[tr + (kk - 1) * ntrain];
    ...
    fastpls_candidate_insert(sim, top_vals, use_k);
  }
...
void fastpls_cuda_candidate_knn_scores_prefix(const double* Ttest,
                                              ...
                                              cudaStream_t stream) {
  const int threads = 256;
  const int blocks = (ntest * top_m * nslice + threads - 1) / threads;
  fastpls_candidate_knn_scores_prefix_kernel<<<blocks, threads, 0, stream>>>(
    Ttest,
    Ttrain,
    test_norm2,
    train_norm2,
