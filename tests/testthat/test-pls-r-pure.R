test_that("pls_r runs both plssvd and simpls with bundled RSVD", {
  set.seed(2026)
  X <- matrix(rnorm(120 * 15), nrow = 120, ncol = 15)
  Y <- matrix(rnorm(120 * 4), nrow = 120, ncol = 4)

  m1 <- pls_r(X, Y, ncomp = 1:3, fit = TRUE, method = "plssvd", svd.method = "cpu_rsvd")
  m2 <- pls_r(X, Y, ncomp = 1:3, fit = TRUE, method = "simpls", svd.method = "cpu_rsvd")
...
test_that("pls_r plssvd handles ncomp above rank by capping internally", {
  ...
  expect_warning({
    fit <- pls_r(X[-idx, ], y[-idx], X[idx, ], ncomp = 50, method = "plssvd", svd.method = "cpu_rsvd")
