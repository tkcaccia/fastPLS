test_that("fastPLS backend precedence is explicit, option, environment, CPU", {
  ...
  options(backend = NULL, fastPLS.backend = NULL); Sys.unsetenv(c("BACKEND", "FASTPLS_BACKEND"))
  expect_identical(fastPLS_backend(), "cpu")
  Sys.setenv(BACKEND = "metal"); expect_identical(fastPLS_backend(), "metal")
  options(backend = "cuda", fastPLS.backend = "cpu"); expect_identical(fastPLS_backend(), "cuda")
