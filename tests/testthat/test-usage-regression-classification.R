...
test_that("SVD utilities and helper functions are usable in practice", {
  ...
  expect_true(all(c("irlba", "dc", "cpu_rsvd", "cuda_rsvd") %in% sm$method))

  sr <- svd_run(A, k = 4, method = "dc")
  ...
  sb <- svd_benchmark(A, k = 4, methods = c("irlba", "dc", "cpu_rsvd"), reps = 2L)
  ...
  model_uni <- pls(A, matrix(rnorm(nrow(A)), ncol = 1), ncomp = 1:3, method = "simpls", svd.method = "dc")
  vip_uni <- ViP(model_uni)
  expect_true(is.matrix(vip_uni))

  model_multi <- pls(A, matrix(rnorm(nrow(A) * 2), ncol = 2), ncomp = 1:3, method = "simpls", svd.method = "dc")
