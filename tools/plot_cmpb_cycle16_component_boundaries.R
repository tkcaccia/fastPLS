...
selected_input <- file.path(evidence, "selected_backend_cycle16_chosen.csv")
selection_dir <- file.path(evidence, "nmr_plssvd_extended_lower_grid", "results")
simpls_dir <- file.path(
...
selected <- utils::read.csv(selected_input, check.names = FALSE)
selected <- selected[
...
draw_selected <- function(device) {
  ...
  graphics::mtext(
    "Outer-test performance at the best training-validation value within each evaluated grid",
    side = 3, outer = TRUE, line = 2.1, font = 2, cex = 1.12
  )
  graphics::mtext(
    "* tested-grid boundary or response-rank limit; these values are not claimed as global optima.",
...
plssvd_raw <- utils::read.csv(
  file.path(selection_dir, "nmr_component_selection_raw.csv"),
  check.names = FALSE
)
simpls_raw <- utils::read.csv(
  file.path(simpls_dir, "nmr_component_selection_raw.csv"),
  check.names = FALSE
)
plssvd_summary <- utils::read.csv(
  file.path(selection_dir, "nmr_component_selection_summary.csv"),
  check.names = FALSE
)
simpls_summary <- utils::read.csv(
  file.path(simpls_dir, "nmr_component_selection_summary.csv"),
  check.names = FALSE
)

draw_selection_panel <- function(raw, summary, selected_k, label, color, log_x) {
  split_ids <- sort(unique(raw$split[raw$status == "ok"]))
  xlim <- range(summary$ncomp)
  ylim <- range(raw$RMSD[raw$status == "ok"], finite = TRUE)
  graphics::plot(
    summary$ncomp, summary$RMSD_median, type = "n",
    log = if (log_x) "x" else "", xaxt = "n",
    xlab = "Number of components", ylab = "Validation RMSD",
    xlim = xlim, ylim = ylim, main = label
  )
  ticks <- summary$ncomp
  if (length(ticks) > 12L) {
  ...
  graphics::polygon(
    ...
    c(summary$RMSD_q25, rev(summary$RMSD_q75)),
    border = NA, col = grDevices::adjustcolor(color, alpha.f = 0.18)
  )
  graphics::lines(
    summary$ncomp, summary$RMSD_median, col = color, lwd = 2.2
  )
  graphics::points(
    summary$ncomp, summary$RMSD_median, pch = 21, bg = color,
    col = "black", cex = 0.9
  )
  graphics::abline(v = selected_k, lty = 2, col = "#333333", lwd = 1.2)
  chosen <- summary[summary$ncomp == selected_k, , drop = FALSE]
  graphics::legend(
    "topright",
    legend = sprintf(
      "Best within grid: k=%d\nMedian RMSD=%.7f",
      selected_k, chosen$RMSD_median
    ),
    bty = "n", cex = 0.82
  )
  graphics::box(bty = "l")
...
draw_nmr_selection <- function(device) {
  ...
  draw_selection_panel(
    plssvd_raw, plssvd_summary, 5L,
    "A  PLS-SVD (extended lower grid)", "#0072B2", TRUE
  )
  draw_selection_panel(
    simpls_raw, simpls_summary, 100L,
    "B  SIMPLS", "#D55E00", FALSE
  )
  graphics::mtext(
...
draw_nmr_selection(function() {
  grDevices::png(
    file.path(plot_dir, "nmr_component_selection_plssvd_simpls.png"),
    width = 3000, height = 1450, res = 300
  )
})
draw_nmr_selection(function() {
  grDevices::pdf(
    file.path(plot_dir, "nmr_component_selection_plssvd_simpls.pdf"),
