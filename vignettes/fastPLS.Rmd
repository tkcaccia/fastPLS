...
(`classifier = "argmax"`) or by linear discriminant analysis in the retained
score space (`classifier = "lda"`). The LDA head can improve discrimination
when class boundaries are not well represented by the largest raw PLS score.
...
controls. For sensitive analyses, compare several seeds and inspect the
reported diagnostics. The detailed equations, storage identities, executable
pseudocode, and cross-language implementation discussion are presented in the
separate software-methods manuscript; this vignette focuses on using the R
package correctly.
...
macOS uses Apple Accelerate for CPU BLAS/LAPACK. OpenBLAS is not required.
Metal is detected automatically on supported Apple systems. To require Metal
and stop installation if it cannot be compiled, set the following before the
source installation:
...
The OpenBLAS release and selected CPU kernel can materially affect large
matrix products. For reproducible timing, use a current build compiled for the
target processor and record both values. Detecting the OpenBLAS family alone
does not establish that an architecture-appropriate kernel is active.
