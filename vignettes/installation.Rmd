...
Expected CPU-library values are `"Accelerate"`, `"OpenBLAS"`, and
`"R BLAS/LAPACK"`. Performance analyses on Linux or Windows that require
OpenBLAS should proceed only when `fastPLS_blas()` returns `"OpenBLAS"`.
...
For benchmark reports, also record compiler versions, OpenBLAS or Accelerate,
