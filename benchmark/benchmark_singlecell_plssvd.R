...
script_file <- if (length(script_arg)) sub("^--file=", "", script_arg[[1]]) else file.path(getwd(), "benchmark", "benchmark_singlecell_plssvd.R")
...
with_plssvd_flag <- function(value, code) {
...
plot_metric <- function(summary_df, metric_col, ylab, file) {
  ...
  plot(
    ...
    ylab = ylab,
    main = paste("singlecell:", ylab, "plssvd baseline vs optimized")
  )
...
out_dir <- normalizePath(
  Sys.getenv("FASTPLS_SINGLECELL_PLSSVD_BENCH_OUTDIR", file.path(dirname(repo_root), "fastPLS_benchmark_results", "singlecell_plssvd")),
  winslash = "/",
  mustWork = FALSE
)
...
plssvd_cap <- min(nrow(ds$X), ncol(ds$X), nlevels(ds$y))
ncomp_list <- ncomp_list[ncomp_list <= plssvd_cap]
if (!length(ncomp_list)) stop("No valid ncomp values remain after PLSSVD cap.")

methods <- data.frame(
  method = c("plssvd_baseline", "plssvd_optimized"),
  plssvd_flag = c(0L, 1L),
...
for (rep_id in seq_len(reps)) {
  ...
  for (ncomp in ncomp_list) {
    for (i in seq_len(nrow(methods))) {
      ...
      message(sprintf("[singlecell-plssvd] rep=%d ncomp=%d method=%s", rep_id, ncomp, cfg$method))
      fit_res <- tryCatch(
        with_plssvd_flag(cfg$plssvd_flag, {
          elapsed <- system.time({
            model <- pls(
              Xtrain = Xtrain,
              Ytrain = Ytrain,
...
write.csv(res, file.path(out_dir, "singlecell_plssvd_replicates.csv"), row.names = FALSE)

ok <- res[res$status == "ok", , drop = FALSE]
if (!nrow(ok)) stop("All benchmark runs failed. See replicates CSV for details.")
...
baseline_ref <- summary_df[summary_df$method == "plssvd_baseline", c("ncomp", "elapsed_time_seconds_median", "accuracy_value_median")]
...
write.csv(summary_df, file.path(out_dir, "singlecell_plssvd_summary.csv"), row.names = FALSE)

plot_metric(
  summary_df,
  metric_col = "elapsed_time_seconds_median",
  ylab = "Median build time (seconds)",
  file = file.path(out_dir, "singlecell_plssvd_time.png")
)
plot_metric(
  summary_df,
  metric_col = "accuracy_value_median",
  ylab = "Median accuracy",
  file = file.path(out_dir, "singlecell_plssvd_accuracy.png")
)

summary_txt <- file.path(out_dir, "singlecell_plssvd_summary.txt")
sink(summary_txt)
cat("singlecell plssvd baseline vs optimized\n\n")
