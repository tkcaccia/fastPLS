...
run_cfg <- function(incr, reps=5L) {
  Sys.setenv(FASTPLS_FAST_INCREMENTAL=as.character(incr), FASTPLS_FAST_INC_ITERS='2', FASTPLS_FAST_DEFLCACHE='1', FASTPLS_FAST_BLOCK='6', FASTPLS_FAST_CENTER_T='1', FASTPLS_FAST_REORTH_V='0')
  ...
  for (i in seq_len(reps)) {
    ...
    t0 <- proc.time()[3]
    model <- fastPLS:::pls_model2_fast(Xtrain, Ymat, as.integer(100), 1L, FALSE, 4L, 14L, 2L, as.integer(9000+i))
  ...
  data.frame(incremental=incr, train_median_ms=median(train_ms), acc_median=median(acc), train_all=paste(sprintf('%.3f', train_ms), collapse=';'), acc_all=paste(sprintf('%.6f', acc), collapse=';'))
}

out <- rbind(run_cfg(0), run_cfg(1))
write.csv(out, '/Users/stefano/Documents/GPUPLS/bench_incremental_vs_non.csv', row.names = FALSE)
print(out)
