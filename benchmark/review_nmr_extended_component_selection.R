...
grid <- sort(unique(as.integer(strsplit(
  get_arg("grid", "10,25,50,75,100,125,150,165,175,200,250,300"),
  ",", fixed = TRUE
)[[1L]])))
...
script_path <- if (length(script_arg)) {
  ...
  normalizePath("benchmark/review_nmr_extended_component_selection.R", mustWork = TRUE)
}
...
selected_index <- which.min(summary$RMSD_median)
selected_ncomp <- summary$ncomp[[selected_index]]
...
selection <- data.frame(
  ...
  selection_rule = "minimum median validation RMSD across repeated training-only splits",
...
write_fastpls_nmr_manifest(
  protocol$metadata, file.path(out_dir, "nmr_protocol_manifest.txt"),
  extra = list(
    selection_scope = "Xtrain/Ytrain only; Xtest/Ytest never accessed for model selection",
    backend = backend,
    svd_method = "rsvd",
    method = method,
    precision = "float64",
...
saveRDS(
  list(
    protocol = protocol$metadata, raw = raw, summary = summary,
